...
all_recs_sumt <- all_recs %>%
  dplyr::select(Continent, Country, Region) %>%
  group_by(Continent, Country) %>%
  tally()

# Create an indicator of whether record is present in full data set
recs_jn <- dplyr::select(all_recs, Latitude, Longitude, Site) %>%
...
all_recs2 <- all_recs %>%
  ...
  dplyr::select(Latitude, Longitude, Continent, Country, 
...
write.csv(recs_summary, here(out_PCA, "Occurrences_Summary.csv"), 
          row.names = FALSE)

# count number of county records for CONUS
